...
description: Prepare a private dense PDF meeting-prep packet for Rockaway Q / QAQ using the user's connected calendar/account context and the read-only Rockaway Q brain MCP. Use for Rockaway Q or QAQ meeting prep, Q team prep, product/ops/customer/project meeting prep, today's remaining Q meetings, next meeting prep, or what to discuss in an upcoming Rockaway Q call.
---
...
Use only the Rockaway Q brain MCP:
...
1. Use `rockaway-q-qmd.status` if connection/index state is uncertain.
2. Use `rockaway-q-qmd.query` first for broad recall across exact title, recurring-series title, product/project/customer/workflow/ops/initiative handles, key attendees, prior discussion, open items, blockers, next steps, risks, and ownership context. Always scope it with `collections: ["rockaway-q"]`.
3. Use `rockaway-q-qmd.get` for the best QMD source before using retrieved facts.
4. Use `rockaway-q.get_page` for the strongest canonical slug hits when full detail is useful.
5. Use `rockaway-q.get_links` / `rockaway-q.get_backlinks` for the top 1-3 central pages when ownership, relationship, or graph context matters.
6. Use `rockaway-q.memory_lookup`, `rockaway-q.search`, or `rockaway-q.query` only as GBrain fallback when native QMD is weak or unavailable.
...
```bash
"${CLAUDE_SKILL_DIR}/.rockaway-meeting-prep-common/scripts/render_packet.js" --input /path/to/packet.json --out-dir "$HOME/Rockaway Meeting Briefs/q/YYYY-MM-DD" --slug rockaway-q-meeting-prep-YYYY-MM-DD
```

If `${CLAUDE_SKILL_DIR}` is unavailable, look for the renderer in:

```text
$HOME/.codex/skills/rockaway-q-meeting-prep/.rockaway-meeting-prep-common/scripts/render_packet.js
$HOME/.claude/skills/rockaway-q-meeting-prep/.rockaway-meeting-prep-common/scripts/render_packet.js
$HOME/.agents/skills/rockaway-q-meeting-prep/.rockaway-meeting-prep-common/scripts/render_packet.js
```
